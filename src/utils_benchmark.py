...
def get_mem_installed() -> str:
    ...
    return f"{total_memory_bytes:.2} GB"
...
def benchmark_mark_time(file_language_run: str, file_size: int, time: str, proc_speed: str, mem_installed: str) -> str:
    import csv
    import os
    results_header = ["file_name","file_size_mb","time_run","cpu_speed_mhz","total_mem_installed"]
    results_file = 'results.csv'
    result_file_exists = os.path.exists(results_file)
    result = ""
    file_name_run = file_language_run.split("\\")[-1]
    print(file_name_run)
    data_run_results = [file_name_run, file_size, time, proc_speed, mem_installed]
